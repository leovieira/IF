<?php
	...
	if(count($erros)==0){
		...
		$sql = "UPDATE cadastro SET nome_titular='$nome_titular', banco='$banco', agencia='$agencia', num_conta='$num_conta', tipo_conta='$tipo_conta' WHERE id=" . $id . ";";
		$rs = mysqli_query($conexao, $sql);
		if($rs!=false){
			echo "<script> alert('Os dados foram editados com sucesso!'); </script>";
			echo "<a href='exibe_dados.php' style='margin-left: 2px; font-size: 12pt; font-family: Arial;'><b>Deseja ver os dados?</b></a>";
		} else{
			echo "Não foi possível editar os dados!";
